...
public abstract class AdWhirlAdapter {
	...
	private static AdWhirlAdapter getAdapter(AdWhirlLayout adWhirlLayout,
			Ration ration) {
		GmAdWhirlEventAdapterData.debugDefaultAdapter(ration);

		if (GmAdWhirlEventAdapterData.CONST_FITADLANGUAGE_ENABLED) {
			if (adWhirlLayout != null) {
				int contextLanguage = AdWhirlUtil
						.getContextLanguage(adWhirlLayout.getContext());
				int adSupportLanguage = AdWhirlUtil
						.getADSupportLangSet(ration.type);
				if ((contextLanguage & adSupportLanguage) == 0) {
					Log.e("AdWhirl", "Skip Ration:" + ration.name);
					return null;
				}
			}
		}
	...
	public static void handle(AdWhirlLayout adWhirlLayout, Ration ration)
			throws Throwable {
		AdWhirlAdapter adapter = AdWhirlAdapter.getAdapter(adWhirlLayout,
				ration);
		if (adapter != null) {
