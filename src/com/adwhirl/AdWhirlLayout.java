...
public class AdWhirlLayout extends RelativeLayout {
	...
	private GmAdWhirlEventHandler mEventHandler = null;

	public interface IGmOnEventAdapterChangedListener {
		void onEventAdapterChanged(GmEventADType adType);
		void onImpression();
		void onCount();
	}

	public IGmOnEventAdapterChangedListener getOnEventAdapterChangedListener() {
		return mEventHandler.getOnEventAdapterChangedListener();
	}

	public synchronized void setOnEventAdapterChangedListener(
			IGmOnEventAdapterChangedListener ls) {
		mEventHandler.setOnEventAdapterChangedListener(ls);
	}
	...
	public AdWhirlLayout(final Activity context, final String keyAdWhirl,
		...
		mEventHandler = new GmAdWhirlEventHandler(this, null);
	}
	...
	private void rotateAd() {
		...
		GmAdWhirlEventAdapterData.debugCustomEventAdapter(nextRation);
	...
	private ScheduledFuture<?> mSchedulerFeature = null;

	private void cancelScheduler() {
		if (mSchedulerFeature != null) {
			mSchedulerFeature.cancel(false);
			int count = 0;
			while ((count < 10) && !mSchedulerFeature.isCancelled()) {
				try {
					Thread.sleep(30);
			...
			mSchedulerFeature = null;
			Log.d(AdWhirlUtil.ADWHIRL, "scheduler canceled!");
		}
	}
	...
	public synchronized void rotateThreadedDelayed() {
		Log.d(AdWhirlUtil.ADWHIRL, "Will call rotateAd() in " + extra.cycleTime
				+ " seconds");
		cancelScheduler();
		mSchedulerFeature = scheduler.schedule(new RotateAdRunnable(this),
				extra.cycleTime, TimeUnit.SECONDS);
	}
