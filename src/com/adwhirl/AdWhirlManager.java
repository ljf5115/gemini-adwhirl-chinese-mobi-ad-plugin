...
public class AdWhirlManager {
	...
	private static IGmOnCustomConfigListener mOnCustomConfigListener = null;

	public static IGmOnCustomConfigListener getOnCustomConfigListener() {
		return mOnCustomConfigListener;
	}

	public static synchronized void setCustomConfigListener(
			IGmOnCustomConfigListener customConfigListener) {
		mOnCustomConfigListener = customConfigListener;
	}

	public static synchronized String getCustomConfigString() {
		String ret = null;
		if (mOnCustomConfigListener != null) {
			ret = mOnCustomConfigListener.getConfigString();
		}
		return ret;
	}
	...
	public Custom getCustom(String nid) {
		...
		if (extra.locationOn == 1) {
			...
			if (location != null) {
				locationString = String.format(AdWhirlUtil.locationString,
		...
		String url = String
				.format(AdWhirlUtil.urlCustom, this.keyAdWhirl, nid,
						deviceIDHash, localeString, locationString,
						AdWhirlUtil.VERSION);
	...
	private String fetchHttpConfig(String urlConfig,
			SharedPreferences adWhirlPrefs) {
		String ret = null;

		HttpClient httpClient = new DefaultHttpClient();

		String url = String.format(urlConfig, this.keyAdWhirl,
				AdWhirlUtil.VERSION);
		HttpGet httpGet = new HttpGet(url);
		...
		try {
			...
			if (entity != null) {
				...
				editor.putString(PREFS_STRING_CONFIG, ret);
	...
	private String fetchCustomConfig(SharedPreferences adWhirlPrefs) {
		String ret = null;

		ret = getCustomConfigString();
		if (!TextUtils.isEmpty(ret)) {
			SharedPreferences.Editor editor = adWhirlPrefs.edit();
			editor.putString(PREFS_STRING_CONFIG, ret);
			editor.putLong(PREFS_STRING_TIMESTAMP, System.currentTimeMillis());
			editor.commit();
			Log.i(AdWhirlUtil.ADWHIRL, "fetch success");
		} else {
			Log.i(AdWhirlUtil.ADWHIRL, "fetch failed");
		}
		return ret;
	}
	...
	public void fetchConfig() {
		...
		SharedPreferences adWhirlPrefs = context.getSharedPreferences(
		...
		if (jsonString == null
				|| GmAdWhirlEventAdapterData.CONST_FORCEFETCHSERVER_ENABLED
			...
			if (TextUtils.isEmpty(webJsonString)) {
				Log.i(AdWhirlUtil.ADWHIRL, "fetchCustomConfig");
				webJsonString = fetchCustomConfig(adWhirlPrefs);
			}
			if (TextUtils.isEmpty(webJsonString)) {
				Log.i(AdWhirlUtil.ADWHIRL, "fetchHTTPConfig");
				webJsonString = fetchHttpConfig(AdWhirlUtil.urlConfig,
						adWhirlPrefs);
			}
			if (!TextUtils.isEmpty(webJsonString)) {
				jsonString = webJsonString;
			}
			/*
			 * HttpClient httpClient = new DefaultHttpClient();
			 * 
			 * String url = String.format(AdWhirlUtil.urlConfig,
			 * this.keyAdWhirl, AdWhirlUtil.VERSION); HttpGet httpGet = new
			 * HttpGet(url);
			 * 
			 ...
			 * HttpEntity entity = httpResponse.getEntity();
			 * 
			 * if (entity != null) { InputStream inputStream =
			 * entity.getContent(); jsonString =
			 * convertStreamToString(inputStream);
			 * 
			 * SharedPreferences.Editor editor = adWhirlPrefs.edit();
			 * editor.putString(PREFS_STRING_CONFIG, jsonString);
			 * editor.putLong(PREFS_STRING_TIMESTAMP,
			 * System.currentTimeMillis()); editor.commit(); } } catch
			 * (ClientProtocolException e) { Log.e(AdWhirlUtil.ADWHIRL,
			 * "Caught ClientProtocolException in fetchConfig()", e); } catch
			 * (IOException e) { Log.e(AdWhirlUtil.ADWHIRL,
		...
		if (!TextUtils.isEmpty(jsonString)) {
			...
			if (rationsList != null) {
				for (Ration tmpRation : rationsList) {
					// validate ration info
					GmAdWhirlEventAdapterData.validateRation(tmpRation);

					// print ration info
					StringBuilder strBuilder = new StringBuilder(" \n");

					strBuilder.append("    nid:");
					strBuilder.append(tmpRation.nid);
					strBuilder.append("\n");

					strBuilder.append("    type:");
					strBuilder.append(tmpRation.type);
					strBuilder.append("\n");

					strBuilder.append("    name:");
					strBuilder.append(tmpRation.name);
					strBuilder.append("\n");

					strBuilder.append("    weight:");
					strBuilder.append(tmpRation.weight);
					strBuilder.append("\n");

					strBuilder.append("    priority:");
					strBuilder.append(tmpRation.priority);
					strBuilder.append("\n");

					strBuilder.append("    key:");
					strBuilder.append(tmpRation.key);
					strBuilder.append("\n");

					strBuilder.append("    key2:");
					strBuilder.append(tmpRation.key2);
					strBuilder.append("\n");

					strBuilder.append(" \n");

					Log.i(AdWhirlUtil.ADWHIRL, strBuilder.toString());
				}
			}
		}
		if ((rationsList == null) || (rationsList.size() == 0)) {
	...
	private void loadDefaultConfiguration() {
		this.extra = new Extra();
		GmAdWhirlEventAdapterData.initDefaultExtra(this.extra);

		List<Ration> rationsList = new ArrayList<Ration>();
		GmAdWhirlEventAdapterData.initDefaultRationList(rationsList);

		this.totalWeight = 0;
		for (int i = 0; i < rationsList.size(); i++) {
			this.totalWeight += rationsList.get(i).weight;
		}
	...
	public static interface IGmOnCustomConfigListener {
